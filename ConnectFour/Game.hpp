...
protected:
    /*
    checks for a valid game which has a valid playerOne, playerTwo and grid set. If true change the value of status to
    GS_IN_PROGRESS 
    */
    ...
    */
    virtual void checkConnect4(unsigned int indexRow, unsigned int indexColumn, Grid::Cell disc);

    /*
    checks for a connect 4 in the vertical plane.
    */
    virtual int checkVertical(int indexRow,int indexColumn, Grid::Cell disc);

    /*
    checks for a connect 4 in the horizontal plane.
    */
    virtual std::vector<int> checkHorizontal(int indexRow,int indexColumn, Grid::Cell disc);

    /*
    checks for a connect 4 in upper diagonal plane.
    */
    virtual std::vector<int> checkDiagonal1(int indexRow, int indexColumn, Grid::Cell disc);

    /*
    checks for a connect 4 in lower diagonal plane.
    */
    virtual std::vector<int> checkDiagonal2(int indexRow, int indexColumn, Grid::Cell disc);

    /*
    helper function that checks if a train of disc is longer than 4
    */
    virtual bool winningCondition(int connect4);

    /*
    find the row index of the first disc for a specific column
    */
    virtual unsigned int findIndexRow(unsigned int indexColumn);

    /*
    sets the winner according to the players scores
    */
    ...
    unsigned int turn; //Odd =player 1, Even =player 2
};
